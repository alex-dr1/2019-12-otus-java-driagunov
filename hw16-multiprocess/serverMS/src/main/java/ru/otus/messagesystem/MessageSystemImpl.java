...
public final class MessageSystemImpl implements MessageSystem {
    ...
    private final Map<String, MsServer> clientMap = new ConcurrentHashMap<>();
    ...
    private final ExecutorService msgHandler = Executors.newFixedThreadPool(MSG_HANDLER_THREAD_LIMIT,
            new ThreadFactory() {
                ...
                public Thread newThread(Runnable runnable) {
                    Thread thread = new Thread(runnable);
                    thread.setName("msg-handler-" + threadNameSeq.incrementAndGet());
                    return thread;
                }
            });
    ...
    public void start() {
        ...
        clientMap.forEach((s, msClientServer) -> msClientServer.start());
    }
    ...
    @Override
    public void addClient(MsServer msClient) {
    ...
    public void removeClient(String clientId) {
        MsServer removedClient = clientMap.remove(clientId);
    ...
    public void dispose() throws InterruptedException {
        clientMap.forEach((s, clientMap) -> {
//            logger.info("Send stop -> {}", clientMap.getName());
            msgHandler.submit(() -> {
                logger.info("Send stop -> {}", clientMap.getName());
                clientMap.addMapQueueSocketMsg(Message.VOID_MESSAGE);
            });
//            clientMap.addSocketMsgQueueOutPost(Message.VOID_MESSAGE);
         });
        ...
        while (runFlag.get() || !messageQueue.isEmpty()) {
            try {
                ...
                if (msg == Message.VOID_MESSAGE) {
                    ...
                    MsServer clientTo = clientMap.get(msg.getTo());
                    if (clientTo == null) {
                        logger.warn("client not found");
                    } else {
//                        logger.info("{} -> addSocketMsgQueueOutPost({})", clientTo.getName(), msg);
//                        clientTo.addSocketMsgQueueOutPost(msg);
                        msgHandler.submit(() -> clientTo.addMapQueueSocketMsg(msg));
                    }
                }
